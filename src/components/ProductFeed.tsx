...
const ProductFeed = () => {
  ...
  return <div className="flex-1 bg-white">
      {/* Tab navigation */}
      <div className="flex pt-4 pb-2 border-b border-gray-100 py-0 my-0 mx-0 px-0">
        {tabs.map(tab => <button key={tab} onClick={() => setActiveTab(tab)} className={`px-4 py-2 text-sm font-medium rounded-full mr-2 transition-colors ${activeTab === tab ? 'bg-yellow-brand text-black' : 'text-gray-600 hover:text-gray-800'}`}>
            {tab}
          </button>)}
      </div>
