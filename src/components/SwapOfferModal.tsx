...
const SwapOfferModal = ({
  ...
  const handleRequestSwap = () => {
    if (selectedItem || offerPrice) {
      ...
      // Show success toast with modern Gen Z styling, positioned at bottom with gradient animation
      toast({
        title: "🎉 Yay! Gửi thành công rồi! ✨",
        description: `Mình sẽ báo bạn ngay khi ${targetItem.owner} phản hồi nhé! 💫`,
        className: "fixed bottom-4 left-1/2 transform -translate-x-1/2 z-[9999] border-l-4 border-l-yellow-500 bg-gradient-to-r from-white via-yellow-50 to-yellow-100 shadow-xl max-w-xs w-full mx-0 rounded-xl border-yellow-200 animate-[gradient_3s_ease-in-out_infinite] bg-[length:200%_200%]",
        duration: 5000,
      });
