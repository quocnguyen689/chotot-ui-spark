...
const ProductCard: React.FC<ProductCardProps> = ({
  ...
  return <div className="bg-white rounded-lg shadow-sm border border-gray-100 overflow-hidden animate-fade-in">
      {/* Image container */}
      <div className="relative">
        ...
        <button onClick={() => setIsLiked(!isLiked)} className="absolute top-2 right-2 w-8 h-8 bg-white/90 rounded-full flex items-center justify-center shadow-sm">
...
const HeartIcon = ({
  ...
  filled: boolean;
  className: string;
}) => <svg className={className} fill={filled ? '#ef4444' : 'none'} viewBox="0 0 24 24" stroke={filled ? '#ef4444' : 'currentColor'} strokeWidth={2}>
